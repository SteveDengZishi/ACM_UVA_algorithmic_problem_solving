...
#define GET(node) data[node]
    ...
    // Create a segtree from vector which stores the range [begin, end) in its leaf nodes level.
    Segtree(vector<int>::iterator begin, vector<int>::iterator end): n(end - begin) {
    ...
    void build_rec(int node, vector<int>::iterator begin, vector<int>::iterator end) {
        if (end == begin+1) {
            if (data.size() <= node) data.resize(node+1);
            data[node] = *begin;
        } else {
            vector<int>::iterator mid = begin + (end-begin+1)/2;
            build_rec(2*node+1, begin, mid);
            build_rec(2*node+2, mid, end);
            data[node] = product(data[2*node+1], data[2*node+2]);
        }
    }
    ...
    void update_range_rec(int node, int tbegin, int tend, int abegin, int aend, int val) {
        if (tbegin >= abegin && tend <= aend) {
            ...
            data[node] = product(GET(2*node+1), GET(2*node+2));
        }
    }
#endif
    int query_rec(int node, int tbegin, int tend, int abegin, int aend) {
        if (tbegin >= abegin && tend <= aend) {
            ...
            int res = 1;
            if (mid > abegin && tbegin < aend)
                res = product(res, query_rec(2*node+1, tbegin, mid, abegin, aend));
            if (tend > abegin && mid < aend)
                res = product(res, query_rec(2*node+2, mid, tend, abegin, aend));
            return res;
        }
    }
...
#endif
    // Returns productimum in range [begin, end). Indices are 0-based.
